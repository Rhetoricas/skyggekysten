<script lang="ts">
	...
	// Fast type til multiplayer-data
	...
	const tilgaengeligeKarakterer: Karakter[] = [
		...
		{ id: 'explorer', navn: "Eventyreren", ikon: "🤠", startMsg: "🤠 Terrænet er din ven, men du starter uden penge og våben.", startHp: 100, startGuld: 0, sabelLevel: 0, skovlLevel: 2, moveCost: 1, digCost: 2, dmgMod: 1.0, goldMod: 1.0, canRest: true, fordel: "Level 2 skovl giver bonus til udforskning. Graving koster 2 HP.", ulempe: "Mangler kamp-erfaring og guld." },
...
if (data) {
	...
	async function syncTilDb() {
		...
		if (data) {
			...
			await supabase.from('spil_sessioner').update({ spillere: opdateredeSpillere }).eq('rum_kode', rumKode);
		}
	}
	...
	function initialiserGitter() {
		...
		const subTrinListe: string[] = [];
		Object.values(eventBibliotek).forEach(ev => {
			ev.valg.forEach(v => {
				if (v.naesteTrin) subTrinListe.push(v.naesteTrin);
				if (v.udfald) {
					const u = v.udfald;
					if (u.katastrofe.naesteTrin) subTrinListe.push(u.katastrofe.naesteTrin);
					if (u.fiasko.naesteTrin) subTrinListe.push(u.fiasko.naesteTrin);
					if (u.neutral.naesteTrin) subTrinListe.push(u.neutral.naesteTrin);
					if (u.succes.naesteTrin) subTrinListe.push(u.succes.naesteTrin);
					if (u.mirakel.naesteTrin) subTrinListe.push(u.mirakel.naesteTrin);
				}
			});
		});

		let ledigeEvents: Record<string, string[]> = {};
		Object.entries(eventBibliotek).forEach(([key, ev]) => {
			if (!ledigeEvents[ev.biome]) ledigeEvents[ev.biome] = [];
			if (!subTrinListe.includes(key)) ledigeEvents[ev.biome].push(key);
		});
		
		for (let i = 0; i < antal; i++) {
			let f = nytGitter[i];
			if (f.biome !== 'hav' && Math.random() < 0.12) {
				if (ledigeEvents[f.biome] && ledigeEvents[f.biome].length > 0) {
					const randomEventIndex = Math.floor(Math.random() * ledigeEvents[f.biome].length);
					f.eventID = ledigeEvents[f.biome].splice(randomEventIndex, 1)[0];
				}
			}
		}
	...
	function købEllerOpgrader(id: string) {
		...
		if (index > -1) {
			...
			if (guldTotal >= upgradePris) {
				guldTotal -= upgradePris; vare.level += 1; logBesked = `Udstyr forbedret.`; lukEvent(); syncTilDb();
			...
			if (guldTotal >= basePris) {
				...
				logBesked = `Handel gennemført.`; lukEvent(); syncTilDb();
	...
	function flytHex(retning: string) {
		...
		if (nI >= 0 && nI < BREDDE * HOEJDE && Math.abs(k - nK) <= 1 && Math.abs(r - nR) <= 1) { 
			...
			if (f) {
				...
				// TETHER LOGIK: Find bagerste levende spiller
				...
				if (livspoint <= 0) { syncTilDb(); return; }
				if (nK === BREDDE - 2) { gameState = 'win'; syncTilDb(); return; }
...
<style>
	...
	.hex { width: 96px; height: 110px; clip-path: polygon(50% 0%, 100% 25%, 100% 75%, 50% 100%, 0% 75%, 0% 25%); background-color: #2d1e14; background-size: cover; background-position: center; transition: filter 0.3s, opacity 0.3s; }
	.hex.odd { transform: translateX(48px); }
	.hex.unexplored { filter: brightness(0.2) grayscale(1); opacity: 0.4; }
	.hex.active { border: 2px solid #5a7d2a; z-index: 5; box-shadow: inset 0 0 20px rgba(90,125,42,0.8); } 
