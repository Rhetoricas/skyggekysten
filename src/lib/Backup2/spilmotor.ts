...
export function hvil() {
    ...
    const maxLiv = karakter.startHp;

    if (spilTilstand.livspoint >= maxLiv) { 
        spilTilstand.logBesked = "Du hviler og genvinder energi. Dit helbred er optimalt. Du tabte 40 guld."; 
    } else { 
        const manglendeLiv = maxLiv - spilTilstand.livspoint;
        const heletLiv = Math.min(30, manglendeLiv);
        
        spilTilstand.livspoint += heletLiv;
        spilTilstand.logBesked = `Du slapper af. Du får fuld energi og heler sår for ${heletLiv} HP.`; 
    }

    if (Math.random() < 0.2) { 
        spilTilstand.logBesked = "Skygger angriber dig i natten."; 
...
export function initialiserGitter() {
    ...
    function spredBiome(startIndeks: number, type: 'by' | 'marked', maxStr: number) {
        let aabne = [startIndeks];
        let lukkede = new Set<number>();
        let bygget = 0;
        ...
        if (vildmark.includes(felt.biome as string) && Math.random() < 0.008) {
            ...
            if (Math.random() < 0.6) {
                ...
                const pulje = felt.biome === 'marked' 
                    ? markedVarePool 
                    : Object.keys(itemDB).filter(k => itemDB[k].pris > 0 && itemDB[k].type !== 'forbandelse');
