    ...
    <div class="event-boks">
        ...
        {#if !eventState.valgLåst}
            <div class="knap-panel">
                ...
                <button class="valg-btn forlad-valg" onclick={lukEvent}>Forlad stedet</button>
            </div>
        {/if}
    </div>
</div>
...
<style>
    ...
    .valg-btn { 
        padding: 12px; background: #2a2a2a; color: white;
        border: 1px solid #555; cursor: pointer; text-align: left; border-radius: 4px; transition: 0.2s; font-size: 16px;
    }
    .valg-btn:hover { background: #3a3a3a; border-color: #777; }
    .forlad-valg { background: #4a1111; border-color: #661111; }
    .forlad-valg:hover { background: #6a1111; }
